...
r = add_section(r, "4. עיקרון מרכזי: 'יש חיישן' אינו אומר 'אפשר SKIP'")
r = add_pair(r, "",
    "לפני שקובעים החלטת SKIP למשימה, לא מספיק לבדוק \"האם יש חיישן על הציוד הזה\" — צריך לבדוק ספציפית "
    "\"האם החיישן הקיים באמת מכסה את סוג הכשל שהבדיקה הפיזית המקורית נועדה לתפוס\". לדוגמה: חיישן טמפרטורה "
    "וסטטוס מפסקים על לוח חשמל לא בהכרח מזהה נתיך שנשרף במעגל ספציפי, קורוזיה, חיבור רופף או ריח שרוף — "
    "אלה כשלים שהבדיקה הפיזית הידנית נועדה לתפוס ושהחיישנים הקיימים אינם מכסים בהכרח.")
r = add_pair(r, "מסקנה מעשית",
    "כשקיים פער כיסוי כזה, ההחלטה הנכונה היא INSPECT (ביקור פיזי בתדירות מופחתת, ממוקד ספציפית במה שהחיישנים "
    "לא רואים) ולא SKIP מלא — עד שמתווסף חיישן ייעודי שסוגר את הפער הספציפי (למשל חיישן זרם/רציפות לכל מעגל "
    "בנפרד, לא רק טמפ' כללית של הלוח).")
r = add_pair(r, "הבחנה חשובה נוספת",
    "יש להבדיל בין שתי שכבות שונות של בדיקה שקל לבלבל ביניהן: (1) בדיקה שנתית ע\"י גורם מוסמך חיצוני "
    "(למשל בודק חשמל מוסמך) — זו בדרך כלל מוגדרת בחוק/תקנות, ונשארת KEEP ללא תלות בטכנולוגיה. "
    "(2) סיור פנימי שוטף של צוות האחזקה (יומי/שבועי) — לרוב אין לו תקן חיצוני שמגדיר תדירות/פרוטוקול מדויק, "
    "והוא הצטבר כנוהל/Best Practice. שכבה (2) היא המועמדת הטבעית לצמצום, אך מהסיבה הטכנית שבסעיף הזה "
    "(פער כיסוי חיישנים) — לא מהיעדר חובה רגולטורית.")
r += 1

r = add_section(r, "5. הסבר עמודות מפת ההחלטות")
cols_explain = [
    ...
    ("זמן ביצוע (דק') / מופעים חזויים / שעות בשנה", "הערכות MVP לחישוב חיסכון פוטנציאלי — ראו הנחות עבודה בסעיף 6."),
]
for k, v in cols_explain:
    r = add_pair(r, k, v)
r += 1

r = add_section(r, "6. הנחות עבודה ומגבלות (MVP)")
assumptions = [
...
r = add_section(r, "7. השלבים הבאים (מתוך מסמך החזון + ממצאי הסיור במגדלי הארבעה)")
next_steps = [
    ...
    "בירור טכני מול G1 הוטלו (מתחזקי מערכת הבקרה, פלטפורמת Trend/IQ-4): קיום ממשק BACnet/OPC או גישת קריאה למסד הנתונים/Trend log.",
    "בירור מול Visitt לגבי גישת API/ייצוא להיסטוריית המשימות שכבר קיימת בפועל בבניין — מקור נתוני אמת מהימן לכיול המודל.",
    "מיפוי פערי כיסוי חיישנים לכל שורת INSPECT (עיקרון סעיף 4) — לא רק \"יש בקר\" אלא \"אילו כשלים ספציפיים הבקר הקיים באמת מזהה\".",
